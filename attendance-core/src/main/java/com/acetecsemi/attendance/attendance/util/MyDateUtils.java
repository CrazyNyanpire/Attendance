package com.acetecsemi.attendance.attendance.util;
...
public class MyDateUtils {
	...
	public static String getNewDayNumDate(String strDate, int days) {
		...
		return MyDateUtils.formaterDate(d.getTime(), MyDateUtils.DefFormatString);
	}
	...
	public static List<String> getDateList(String startDate, String endDate)
		...
		day = day / (60 * 60 * 24); // 天数
		day = MyDateUtils.getDayNumBetween2Date(start,end);
		for (int i = 0; i <= day; i++) {
    ...
    public static String addDays(String dateStr, int amount) {
    	Date date = str2Date(dateStr,DefFormatString);
        return formaterDate(add(date, Calendar.DAY_OF_MONTH, amount),DefFormatString);
    }
    
    public static String add(String dateStr, int calendarField, int amount) {
        if (dateStr == null) {
            throw new IllegalArgumentException("The date must not be null");
        }
        Date date = str2Date(dateStr,DefFormatString);
        return formaterDate(add(date, calendarField, amount),DefFormatString);
    }
