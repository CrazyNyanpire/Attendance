		...
		_initHead : function() {
			...
			sorts.on('click', function(e) {
				...
				if ($this.hasClass('sorting-asc')) {
					...
					$this.find('span').remove().end().append($('<span class="glyphicon glyphicon-chevron-down"></span>'));
					self.sortOrder = 'desc';
				} else {
					sorts.removeClass('sorting-asc').removeClass('sorting-desc').find('span').remove();
					$this.removeClass('sorting-desc').addClass('sorting-asc');
					$this.find('span').remove().end().append($('<span class="glyphicon glyphicon-chevron-up"></span>'));
					self.sortOrder = 'asc';
				}
				self._loadData();
			});
