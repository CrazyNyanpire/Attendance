...
@Named
@Transactional
public class MonthlyAttendanceConfirmationCreateApplicationImpl implements
		MonthlyAttendanceConfirmationCreateApplication {

	Logger log = Logger
			.getLogger(MonthlyAttendanceConfirmationCreateApplicationImpl.class);
	...
	public String createMonthlyAttendanceConfirmation(int year, int month) {
		String result = "";
		year = 0;
		month = 5;
	...
	private List<MonthlyAttendanceConfirmation> queryMonthlyAttendanceConfirmationList(
		...
		String startDate = "";
		String endDate = "";
		MyDateUtils.getToday();
		if(MyDateUtils.getToday().compareTo(timestamp.getTimestamp()) < 0 ){
			return new ArrayList<MonthlyAttendanceConfirmation>();
		}
		...
		jpql.append("白班应出勤,");
		jpql.append("count(CASE WHEN attendenceType = '1' and attendenceInType is not null THEN 1 ELSE NULL END) + (CASE WHEN 白班出勤 is NULL THEN 0 ELSE 白班出勤 END)  白班实际出勤,");
		...
		jpql.append("夜班应出勤,");
		jpql.append("count(CASE WHEN attendenceType = '2' and  attendenceInType is not null THEN 1 ELSE NULL END) + (CASE WHEN 夜班出勤 is NULL THEN 0 ELSE 夜班出勤 END)  夜班实际出勤,");
		...
		jpql.append("长白班应出勤,");
		jpql.append("count(CASE WHEN a.duty_type = '1' and (attendenceInType is not null and attendenceInType <> '10') THEN 1 ELSE NULL END)  长白班实际出勤,");
		...
		for (Object[] object : mtcList) {
			...
			double annualVacationBalance = Double.parseDouble(map.get(45)) - Double.parseDouble(map.get(36));
